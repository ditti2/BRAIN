...
\item {\texttt{useBRAIN2}}, which computes the probabilities of aggregated isotopic variants for biomolecules composed out of carbon, hydrogen, oxygen, nitrogen and sulfur. This function allows for heuristics aimed for time savings in computations.
...
\section{BRAIN 2.0 heuristics}\label{BRAIN2}
As mentioned in \textit{Dittwald P., Valkenborg D. BRAIN 2.0: time and memory complexity improvements in the algorithm for calculating the isotope distribution. JASMS, 2014}, it is possible to use several heuristics to make faster computations related to isotopic distribution computations.

\subsection{RCL: Recurrence of Constant Length}\label{RCL}
The recurrence used to compute isotopic distribution might be shortened by using \textit{approxParam}, with value >= 5 should be enough. This limits the number of multiplications, without much loss in accuracy. Let us see the examples:
...
\subsection{LSP: Late Starting Point}\label{LSP}
By using \textit{approxStart} parameter, we can narrow down computations to the peak indices arrount most abundant ones. 
This option changes absolute values of isotopic distribution, but is aimed to approximate peak ratios.
Note, the approximation needs some burn-in period to retrieve good isotopic ratios. The exemplary rule-of-thumb suggested in \textit{BRAIN 2.0} paper is $$\lceil(\log_{10}(monoisotopicMass)\rceil + 5$$
Also, LSP option is useful for heavy molecules, were most abundant peak is much shifted from the monoisotopic peak.
<<>>=
